...
class _DialogoHorometroState extends State<DialogoHorometro> {
  ...
  // 🔥 NUEVO: Método para cerrar el diálogo (sin guardar)
  void _cerrarSinGuardar() {
    Navigator.pop(context);
  }

  // 🔥 MODIFICADO: Guardar y luego cerrar
  Future<void> _guardarHorometros() async {
    ...
    if (guardado) {
      ...
      Navigator.pop(context, true); // Cerrar después de guardar
    } else {
      _mostrarSnackbar('Error al guardar los horómetros', Colors.red);
    }
  }

  // 🔥 NUEVO: Método para guardar al hacer clic fuera
  Future<void> _guardarAlCerrar() async {
    ...
    if (_hayErrores()) {
      // Si hay errores, no guardar y mostrar mensaje
    ...
    if (guardado) {
      ...
      _mostrarSnackbar('Error al guardar los horómetros', Colors.red);
      // Si hay error, no cerrar el diálogo
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // 🔥 ENVOLVER con PopScope para interceptar el cierre por clic fuera
    return PopScope(
      canPop: false, // No permitir cierre directo
      onPopInvoked: (bool didPop) async {
        if (didPop) return;
        // Cuando se intenta cerrar (clic fuera o botón atrás)
        if (isEditable && !_isSaving) {
          await _guardarAlCerrar();
        } else {
          // Si no es editable, cerrar normalmente
          Navigator.pop(context);
        }
      },
      child: Dialog(
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    TextButton(
                      onPressed: _cerrarSinGuardar, // 🔥 Cancelar: solo cierra sin guardar
  ...
  // ... (todos los métodos _buildCardsView, _buildCardField, _buildCardCheckbox, 
  // _buildTable, _buildHeaderCell, _buildCell, _buildEditableCell, _buildCheckboxCell 
  // se mantienen IGUALES que en tu código original)
  
  // Incluye aquí todos los métodos de construcción que ya tenías sin cambios
