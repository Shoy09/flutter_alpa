  ...
  try {
    ...
    setState(() {
      ...
      if (widget.operacionExistente != null) {
        ...
        // Auto-seleccionar si solo hay un equipo y no es una operación existente
...
void _autoSelectSingleEquipo() {
  ...
  if (equipos.length == 1 && selectedEquipo == null) {
    setState(() {
      selectedEquipo = equipos.first;
      // Actualizar los códigos filtrados para ese equipo
      codigosFiltrados = codigosPorEquipo[selectedEquipo] ?? [];
...
void _autoSelectSingleCodigo() {
  if (codigosFiltrados.length == 1 && selectedCodigo == null) {
    setState(() {
      selectedCodigo = codigosFiltrados.first;
      _actualizarCapacidadesPorCodigo(selectedCodigo!);
    });
  }
}
  ...
  void _autoSelectAllTiposEquipo() {
  if (tiposEquipo.isEmpty) return;
  
  bool hayAlgunoSeleccionado = tiposSeleccionados.values.contains(true);
  
  // Solo auto-seleccionar si no hay ninguno seleccionado aún
  if (!hayAlgunoSeleccionado) {
    ...
    print('Todos los tipos de equipo seleccionados automáticamente');
  }
}
  ...
  try {
    ...
    setState(() {
      ...
      if (widget.operacionExistente != null) {
        ...
        // Auto-seleccionar todos los tipos si no es una operación existente
        _autoSelectAllTiposEquipo();
      }
    });
...
Widget _buildDesktopLayout(double cardWidth) {
  Map<String, double> fieldWeights = {
    'fecha': 1.5,      // Aumentado
    'turno': 1.5,      // Aumentado
    'codigo': 1.5,     // Aumentado
    'operador': 1.5,   // Mantener
    'jefe': 1.5,       // Aumentado
    'seccion': 1.5,    // Aumentado
  };
