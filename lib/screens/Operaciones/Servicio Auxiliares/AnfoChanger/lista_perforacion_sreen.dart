...
class TaladroAnfoChangerScreen extends StatefulWidget {
  ...
  const TaladroAnfoChangerScreen({Key? key, this.rolUsuario, this.dniUsuario}) : super(key: key);

  @override
  State<TaladroAnfoChangerScreen> createState() => _TaladroAnfoChangerScreenState();
}
...
Future<void> _cargarEstadosOperacion() async {
  ...
  List<Map<String, dynamic>> estados =
      await DatabaseHelper().getEstadosByOperacionIdAnfochanger(operacionId!);
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    int filasAfectadas =
                        await DatabaseHelper().eliminarOperacionTalAnfochangerFisico(operacionId!);
...
Future<void> _crearRegistroEstado(Map<String, dynamic> data, String estado) async {
  ...
  'observaciones': data['observaciones'] ?? '',
    ...
    Map<String, dynamic>? nuevoEstado = await DatabaseHelper().createEstadoAnfochanger(
      operacionActual!['id'],
      estado,
      data['codigo']!,
      data['hora_inicio']!,
...
Future<List<Map<String, dynamic>>> _getOperaciones() async {
  if (operacionActual == null) return [];
  return await DatabaseHelper().getEstadosByOperacionIdAnfochanger(operacionActual!['id']);
}
    ...
    builder: (BuildContext context) {
      return DialogoFormularioAnfochanger(
        ...
        onGuardar: (datosActualizados) async {
          bool guardado = await DatabaseHelper().updateOperacionByEstadoIdAnfochanger(
            operacionActual!['id'],
            estadoId,
            datosActualizados,
          );
...
Future<void> _handleNuevaOperacion(Map<String, dynamic> data) async {
  ...
  List<Map<String, dynamic>> horometros =
      await dbHelper.getHorometrosPorOperacion('anfochanger');
  ...
  List<Map<String, dynamic>> checklistItems =
      await dbHelper.getCheckListByProceso('ANFOCHANGER');
...
Future<void> _editarOperacionOperativo(Map<String, dynamic> operacion) async {
  Map<String, dynamic> datosAnfochanger = await DatabaseHelper()
      .getOperacionByEstadoIdAnfochanger(operacionActual!['id'], operacion['id']);
    ...
    builder: (BuildContext context) {
      return DialogoFormularioAnfochanger(
        ...
        onGuardar: (datosActualizados) async {
          ...
          if (guardado) {
            _mostrarSnackBar("Datos de Anfochanger actualizados", Colors.green);
            await _fetchOperacionData();
          } else {
        ...
        onConfirmar: () async {
          ...
          try {
            var ultimoEstado = await DatabaseHelper()
                .getUltimoEstadoByOperacionIdAnfochanger(operacionActual!['id']);
            ...
            bool actualizado = await DatabaseHelper().updateHoraFinalAnfochanger(
              operacionActual!['id'],
              ultimoEstado['id'],
              horaReservaInicio,
            );
            ...
            await DatabaseHelper().cerrarOperacionAnfochanger(operacionActual!['id']);
