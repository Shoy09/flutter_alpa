...
import 'package:i_miner/screens/Operaciones/Servicio%20Auxiliares/Rompebancos/widgets/dialogo_no_operativo_formulario_perforacion.dart' show DialogoFormularioNoOpePerforacion;
...
class TaladroRompebancosScreen extends StatefulWidget {
  ...
  const TaladroRompebancosScreen({Key? key, this.rolUsuario, this.dniUsuario}) : super(key: key);

  @override
  State<TaladroRompebancosScreen> createState() => _TaladroRompebancosScreenState();
}
...
Future<void> _cargarEstadosOperacion() async {
  ...
  List<Map<String, dynamic>> estados =
      await DatabaseHelper().getEstadosByOperacionIdRompeBaco(operacionId!);
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    int filasAfectadas =
                        await DatabaseHelper().eliminarOperacionTalRompeBacoFisico(operacionId!);
...
Future<void> _crearRegistroEstado(Map<String, dynamic> data, String estado) async {
  try {
    ...
    Map<String, dynamic>? nuevoEstado = await DatabaseHelper().createEstadoRompeBaco(
      operacionActual!['id'],
      estado,
      data['codigo']!,
      data['hora_inicio']!,
...
Future<List<Map<String, dynamic>>> _getOperaciones() async {
  if (operacionActual == null) return [];
  return await DatabaseHelper().getEstadosByOperacionIdRompeBaco(operacionActual!['id']);
}
    ...
    builder: (BuildContext context) {
      return DialogoFormularioRompebanco(
        ...
        onGuardar: (datosActualizados) async {
          bool guardado = await DatabaseHelper().updateOperacionByEstadoIdRompeBaco(
            operacionActual!['id'],
            estadoId,
            datosActualizados,
          );
...
Future<void> _handleNuevaOperacion(Map<String, dynamic> data) async {
  ...
  print('   equipo: ${data['equipo']}');
   ...
   List<Map<String, dynamic>> horometros =
      await dbHelper.getHorometrosPorOperacion('rompebanco');
  ...
  List<Map<String, dynamic>> checklistItems =
      await DatabaseHelper().getCheckListByProceso('ROMPEBANCOS');
...
Future<void> _editarOperacionOperativo(Map<String, dynamic> operacion) async {
  Map<String, dynamic> datosRompebanco = await DatabaseHelper()
      .getOperacionByEstadoIdRompeBaco(operacionActual!['id'], operacion['id']);
    ...
    builder: (BuildContext context) {
      return DialogoFormularioRompebanco(
        ...
        onGuardar: (datosActualizados) async {
          ...
          if (guardado) {
            _mostrarSnackBar("Datos de Rompebanco actualizados", Colors.green);
            await _fetchOperacionData();
          } else {
        ...
        onConfirmar: () async {
          ...
          try {
            var ultimoEstado = await DatabaseHelper()
                .getUltimoEstadoByOperacionIdRompeBaco(operacionActual!['id']);
            ...
            bool actualizado = await DatabaseHelper().updateHoraFinalRompeBaco(
              operacionActual!['id'],
              ultimoEstado['id'],
              horaReservaInicio,
            );
            ...
            await DatabaseHelper().cerrarOperacionRompeBaco(operacionActual!['id']);
