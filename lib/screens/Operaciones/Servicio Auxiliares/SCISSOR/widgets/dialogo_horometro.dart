...
class _DialogoHorometroState extends State<DialogoHorometro> {
  ...
  Future<void> _cargarHorometros() async {
    setState(() {
      // Obtener datos de horometro_principal y diesel
  ...
  Future<void> _guardarHorometros() async {
    ...
    // Construir el mapa de horómetros para guardar (Horómetro Principal y Diesel)
    ...
    bool guardado = await DatabaseHelper().updateHorometrosScissor(
      widget.operacionId,
      horometrosToSave,
    );
  ...
  Widget _buildTable() {
    return Container(
      ...
      child: Table(
        ...
        columnWidths: const {
          0: FixedColumnWidth(180), // Aumentado para "Horómetro Principal"
        ...
        children: [
          ...
          // Rows - Horómetro Principal y Diesel
          for (int i = 0; i < horometros.length; i++)
            TableRow(
              decoration: BoxDecoration(
