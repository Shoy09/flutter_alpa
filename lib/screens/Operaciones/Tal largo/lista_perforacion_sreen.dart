...
class TaladroLargoScreen extends StatefulWidget {
  ...
  const TaladroLargoScreen({Key? key, this.rolUsuario, this.dniUsuario}) : super(key: key);
...
void obtenerEstadosBD() async {
  estadosBD = await DatabaseHelper().getEstadosBD('PERFORACIÓN TALADROS LARGOS');
...
Future<void> _cargarEstadosOperacion() async {
  ...
  List<Map<String, dynamic>> estados =
      await DatabaseHelper().getEstadosByOperacionId(operacionId!);
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    int filasAfectadas =
                        await DatabaseHelper().eliminarOperacionTalLargoFisico(operacionId!);
...
void _mostrarDialogoEstado(String estado) async {
  ...
  for (var e in todosLosEstados) {
    print("ID:${e['id']} | N°:${e['numero']} | Hora:${e['hora_inicio']} | Estado:${e['estado']}");
  }
...
Future<List<Map<String, dynamic>>> _getOperaciones() async {
  if (operacionActual == null) return [];
  return await DatabaseHelper().getEstadosByOperacionId(operacionActual!['id']);
}
...
Future<void> _handleNuevaOperacion(Map<String, dynamic> data) async {
  ...
  List<Map<String, dynamic>> horometros =
      await dbHelper.getHorometrosPorOperacion('tal_largo');
