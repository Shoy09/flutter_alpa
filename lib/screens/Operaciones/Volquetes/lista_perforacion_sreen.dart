...
import 'package:i_miner/screens/Operaciones/Volquetes/widgets/dialogo_confirmar_cierre.dart';
import 'package:i_miner/screens/Operaciones/Volquetes/widgets/dialogo_formulario_perforacion.dart';
import 'package:i_miner/screens/Operaciones/Volquetes/widgets/dialogo_horometro.dart';
import 'package:i_miner/screens/Operaciones/Volquetes/widgets/dialogo_no_operativo_formulario_perforacion.dart';
import 'package:i_miner/screens/Operaciones/Volquetes/widgets/show_registro_operacion.dart';
...
class TaladroVolquetesScreen extends StatefulWidget {
  final String? rolUsuario;
  final String? dniUsuario;

  const TaladroVolquetesScreen({Key? key, this.rolUsuario, this.dniUsuario}) : super(key: key);

  @override
  State<TaladroVolquetesScreen> createState() => _TaladroVolquetesScreenState();
}

class _TaladroVolquetesScreenState extends State<TaladroVolquetesScreen> {
...
void obtenerEstadosBD() async {
  estadosBD = await DatabaseHelper().getEstadosBD('ACARREO');
...
Future<void> _fetchOperacionData() async {
  ...
  if (widget.rolUsuario == 'Master') {
    data = await dbHelper.getOperacionVolquetesByTurnoAndFechaMaster(
        selectedTurno!, fechaActual);
  } else {
    data = await dbHelper.getOperacionVolquetesByTurnoAndFecha(
        selectedTurno!, fechaActual);
  }
...
Future<void> _cargarEstadosOperacion() async {
  ...
  List<Map<String, dynamic>> estados =
      await DatabaseHelper().getEstadosByOperacionIdVolquetes(operacionId!);
          ...
          sliver: SliverList(
            delegate: SliverChildListDelegate([
              ...
              Container(
                ...
                child: TablaOperacionesVolquetes(
              ...
              // Botones inferiores
              BotonesAccionesInferioresVolquetes(
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    int filasAfectadas =
                        await DatabaseHelper().eliminarOperacionTalVolquetesFisico(operacionId!);
...
void _mostrarDialogoEstado(String estado) async {
  ...
  List<Map<String, dynamic>> todosLosEstados = await DatabaseHelper()
      .getEstadosByOperacionIdVolquetes(operacionActual!['id']);
  ...
  final result = await showRegistroOperacionDialogVolquetes(
    context: context,
...
Future<void> _crearRegistroEstado(Map<String, dynamic> data, String estado) async {
  try {
    ...
    if (ultimoEstadoActivo != null) {
      await DatabaseHelper().updateHoraFinalVolquetes(
        operacionActual!['id'],
    ...
    Map<String, dynamic>? nuevoEstado = await DatabaseHelper().createEstadoVolquetes(
      operacionActual!['id'],
      estado,
      data['codigo']!,
      data['hora_inicio']!,
...
Future<void> _actualizarRegistroEstado(Map<String, dynamic> data, Map<String, dynamic> operacionOriginal) async {
  try {
    ...
    bool actualizado = await DatabaseHelper().updateEstadoVolquetes(
      operacionActual!['id'],
      data['id'],
      numero: data['numero'],
      estado: data['estado'],
...
void _mostrarDialogoSecundario(int estadoId, String estado, String codigo) async {
  ...
  Future.delayed(Duration.zero, () {
    if (estado == "OPERATIVO") {
      _abrirDialogoOperativoVolquetes(estadoId, codigo);  // ← PASAR CÓDIGO
    } else {
      _abrirDialogoNoOperativoVolquetes(estadoId, estado);  // ← PASAR CÓDIGO
    }
  });
}

Future<void> _abrirDialogoOperativoVolquetes(int estadoId, String codigo) async {
  Map<String, dynamic> datos = await DatabaseHelper()
      .getOperacionByEstadoIdVolquetes(operacionActual!['id'], estadoId);

  showDialog(
    context: context,
    builder: (BuildContext context) {
      return DialogoFormularioPerforacionVolquetes(
        ...
        onGuardar: (datosActualizados) async {
          bool guardado = await DatabaseHelper()
              .updateOperacionByEstadoIdVolquetes(
            operacionActual!['id'],
            estadoId,
            datosActualizados,
          );
...
void _abrirDialogoNoOperativoVolquetes(int estadoId, String estado) async {
  Map<String, dynamic> datos = await DatabaseHelper()
      .getOperacionByEstadoIdVolquetes(operacionActual!['id'], estadoId);
  ...
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return DialogoFormularioNoOpeVolquetes(
        operacionId: operacionActual!['id'],
        estadoId: estadoId,
        estado: estado,
        datosIniciales: datos,
...
Future<void> _handleNuevaOperacion(Map<String, dynamic> data) async {
  ...
  /// Insertar operación con los nuevos campos
  await dbHelper.insertOperacionVolquetes(
    data['fecha'],
    data['turno'],
    data['guardia'],        // ← cambió de 'seccion' a 'guardia'
    data['operador'],
    data['jefeGuardia'],    // ← cambió de 'jefe_guardia' a 'jefeGuardia'
    data['nVolquete'],      // ← cambió de 'equipo' y 'n_equipo' a 'nVolquete'
    data['empresa'],        // ← nuevo campo
    null,                   // ← horometrosBase (por ahora null)
  );
...
void _editarOperacion(Map<String, dynamic> operacion) async {
  ...
  try {
    ...
    if (estado == "OPERATIVO") {
      await _editarOperacionOperativoVolquetes(operacion);
    } else {
      await _editarOperacionNoOperativoVolquetes(operacion, estado);
    }
  } catch (e) {
    if (Navigator.canPop(context)) Navigator.pop(context);
...
Future<void> _editarOperacionOperativoVolquetes(
    Map<String, dynamic> operacion) async {

  Map<String, dynamic> datos = await DatabaseHelper()
      .getOperacionByEstadoIdVolquetes(
...
Future<void> _editarOperacionNoOperativoVolquetes(
...
void _handleHorometro() async {
  ...
  Map<String, dynamic> horometrosData = await DatabaseHelper()
      .getHorometrosByOperacionIdVolquetes(operacionId);

  showDialog(
    context: context,
    builder: (BuildContext context) {
      return DialogoHorometroVolquetes(
...
void _handleCerrarRegistros() {
  ...
  showDialog(
    ...
    builder: (BuildContext dialogContext) {
      return DialogoConfirmarCierreRegistrosVolquetes(
        primaryColor: primaryColor,
        onConfirmar: () async {
          ...
          try {
            var ultimoEstado = await DatabaseHelper()
                .getUltimoEstadoByOperacionIdVolquetes(operacionActual!['id']);
            ...
            bool actualizado = await DatabaseHelper().updateHoraFinalVolquetes(
              operacionActual!['id'],
              ultimoEstado['id'],
              horaReservaInicio,
            );
            ...
            await DatabaseHelper().createReservaEstadoVolquetes(
              operacionActual!['id'],
              newNumber,
              horaReservaInicio,
              horaReservaFinal,
            ...
            await DatabaseHelper().cerrarOperacionVolquetes(operacionActual!['id']);
...
Future<void> _eliminarRegistroEstado(Map<String, dynamic> estado) async {
  try {
    ...
    bool eliminado = await DatabaseHelper().deleteEstadoVolquetes(
      operacionActual!['id'],
      estado['id'],
    );
