...
class TaladroEmpernadorScreen extends StatefulWidget {
  ...
  const TaladroEmpernadorScreen({Key? key, this.rolUsuario, this.dniUsuario}) : super(key: key);

  @override
  State<TaladroEmpernadorScreen> createState() => _TaladroEmpernadorScreenState();
}
...
Future<void> _cargarEstadosOperacion() async {
  ...
  List<Map<String, dynamic>> estados =
      await DatabaseHelper().getEstadosByOperacionIdEmpernador(operacionId!);
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    int filasAfectadas =
                        await DatabaseHelper().eliminarOperacionTalEmpernadorFisico(operacionId!);
...
Future<void> _crearRegistroEstado(Map<String, dynamic> data, String estado) async {
  try {
    ...
    Map<String, dynamic>? nuevoEstado = await DatabaseHelper().createEstadoEmpernador(
      operacionActual!['id'],
      estado,
      data['codigo']!,
      data['hora_inicio']!,
...
Future<List<Map<String, dynamic>>> _getOperaciones() async {
  if (operacionActual == null) return [];
  return await DatabaseHelper().getEstadosByOperacionIdEmpernador(operacionActual!['id']);
}
    ...
    builder: (BuildContext context) {
      return DialogoFormularioEmpernador(
        ...
        onGuardar: (datosActualizados) async {
          bool guardado = await DatabaseHelper().updateOperacionByEstadoIdEmpernador(
            operacionActual!['id'],
            estadoId,
            datosActualizados,
          );
...
Future<void> _handleNuevaOperacion(Map<String, dynamic> data) async {
  ...
  List<Map<String, dynamic>> horometros =
      await dbHelper.getHorometrosPorOperacion('empernador');
...
Future<void> _editarOperacionOperativo(Map<String, dynamic> operacion) async {
  Map<String, dynamic> datosPerforacion = await DatabaseHelper()
      .getOperacionByEstadoIdEmpernador(operacionActual!['id'], operacion['id']);
        ...
        onConfirmar: () async {
          ...
          try {
            var ultimoEstado = await DatabaseHelper()
                .getUltimoEstadoByOperacionIdEmpernador(operacionActual!['id']);
            ...
            bool actualizado = await DatabaseHelper().updateHoraFinalEmpernador(
              operacionActual!['id'],
              ultimoEstado['id'],
              horaReservaInicio,
            );
            ...
            await DatabaseHelper().cerrarOperacionEmpernador(operacionActual!['id']);
